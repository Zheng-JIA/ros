    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Test
                ...
                dt = (rospy.Time.now() - starttime).to_sec()
                cmd.linear.y = 10.0*(self._state[3]) # roll rate  
                cmd.linear.x = 10.0*(self._state[4]) # pitch rate
                cmd.angular.z = 5.0*(self._state[5]) # yaw rate 
                cmd.linear.z = -20000.0*(self._state[2]-1.0) # thrust
                print(cmd.angular.z)
